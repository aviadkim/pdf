/**
 * Ultimate RAG + Memory System - FIXED VERSION
 ...
 * FIX: Proper value candidate detection and RAG-enhanced selection
 */

const fs = require('fs');
const pdf = require('pdf-parse');

class UltimateRAGMemorySystemFixed {
    constructor() {
        this.config = {
            ...
            financialPatterns: {
                ...
                marketValuePatterns: [
                    ...
                    /actual\s*price[^0-9]*([0-9',.]+)/gi,
                    /market\s*value[^0-9]*([0-9',.]+)/gi,
                    /current\s*value[^0-9]*([0-9',.]+)/gi
                ],
                
                // Nominal value indicators (face values - NOT market)
                nominalPatterns: [
                    /nominal[^0-9]*([0-9',.]+)/gi,
                    /face\s*value[^0-9]*([0-9',.]+)/gi,
                    /principal[^0-9]*([0-9',.]+)/gi,
                ...
                summaryPatterns: [
                    /total[^0-9]*([0-9',.]+)/gi,
                    /portfolio\s*total[^0-9]*([0-9',.]+)/gi,
                    /bonds[^0-9]*([0-9',.]+).*(?:\d+\.\d+%)/gi, // Category totals
        ...
        console.log('🧠 ULTIMATE RAG + MEMORY SYSTEM FIXED - INITIALIZED');
        ...
        console.log('🔧 FIX: Proper value candidate detection and RAG-enhanced selection');
    }
    ...
    async performFinancialIntelligence(completeData, ragAnalysis, memoryAnalysis) {
        ...
        for (const isinData of completeData.isins) {
            ...
            // Step 1: Get all value candidates for this ISIN - FIXED
            const valueCandidates = this.getAllValueCandidatesFixed(isinData, completeData);
    ...
    async processPDF(pdfBuffer) {
        console.log('🚀 ULTIMATE RAG + MEMORY PROCESSING (FIXED)');
        ...
        try {
            ...
            return {
                success: true,
                method: 'ultimate_rag_memory_fixed',
    ...
    /**
     * FIXED: Get all value candidates for an ISIN
     */
    getAllValueCandidatesFixed(isinData, completeData) {
        const candidates = [];
        
        // Strategy 1: Find all numbers in the ISIN's line context
        ...
        // Strategy 2: Find numbers in full context around ISIN
        const fullContextNumbers = this.extractNumbersFromContext(isinData.fullContext);
        
        for (const number of fullContextNumbers) {
            if (number.value >= 100 && number.value <= 50000000) {
                candidates.push({
                    value: number.value,
                    source: 'full_context',
                    raw: number.raw,
                    context: isinData.fullContext,
                    confidence: 0.5
                });
            }
        }
        
        // Strategy 3: Look for currency prefixed values (USD, CHF, etc.)
        const currencyPattern = /(?:USD|CHF|EUR|GBP)\s*([0-9',\s]+(?:\.\d{2})?)/gi;
        const currencyMatches = [...isinData.fullContext.matchAll(currencyPattern)];
        ...
        // Strategy 4: Look for specific countervalue patterns
        const countervaluePattern = /countervalue\s*USD\s*([0-9',\s]+(?:\.\d{2})?)/gi;
        const countervalueMatches = [...isinData.fullContext.matchAll(countervaluePattern)];
...
module.exports = { UltimateRAGMemorySystemFixed };

// Test the fixed ultimate system
async function testUltimateRAGMemorySystemFixed() {
    console.log('🚀 TESTING ULTIMATE RAG + MEMORY SYSTEM - FIXED VERSION');
    console.log('Revolutionary: Complete Data → RAG → Memory → Financial Intelligence');
    console.log('Target: 100% ISIN ↔ Quantity ↔ Market Value accuracy');
    console.log('🔧 FIX: Proper value candidate detection and RAG-enhanced selection');
    console.log('=' * 75);
    
    const system = new UltimateRAGMemorySystemFixed();
    ...
    if (results.success) {
        console.log('\n✅ ULTIMATE RAG + MEMORY SYSTEM (FIXED) SUCCESS!');
        console.log('=' * 60);
        ...
        console.log(`   Ultimate RAG + Memory (Fixed): ${results.accuracy.toFixed(2)}%`);
        ...
        const resultsFile = `ultimate_rag_memory_fixed_results_${timestamp}.json`;
        ...
        console.log('✅ Fixed Value Detection - Proper candidate identification and classification');
...
if (require.main === module) {
    testUltimateRAGMemorySystemFixed().catch(console.error);
}