...
async function testPlatformFeatures() {
    ...
    try {
        ...
        const testContent = 'Test PDF content for platform demonstration';
        const testFile = Buffer.from(testContent);
        form.append('file', testFile, 'test-document.pdf');
        
        const uploadResponse = await axios.post(baseUrl + '/api/extract-comprehensive', form, {
            headers: form.getHeaders(),
            validateStatus: () => true
        });
        
        console.log(`  Upload Test: ${uploadResponse.status === 200 ? '✅ Success' : '⚠️ Failed'}`);
