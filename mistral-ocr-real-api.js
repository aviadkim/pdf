    ...
    async prepareInputForAPI(input, inputType) {
        ...
        let requestBody = {
            ...
            messages: [
                {
                    role: 'user',
                    content: `Please analyze this financial document and extract all securities with their ISINs and market values. 

Pay special attention to:
- ISIN codes (format: 2 letters + 10 alphanumeric characters)
- Market values in Swiss format (with apostrophes: 1'234'567)
- Table structures with securities data
- Portfolio totals and percentages

Return the extracted text in structured markdown format, preserving table layouts and formatting.`
                }
            ]
        };
        
        // For now, let's use the simulation approach since the exact API format needs clarification
        // In a real implementation, we would need to verify the exact Mistral OCR API format
        
        if (inputType === 'url') {
            console.log('   🌐 URL input prepared');
        } else if (inputType === 'base64') {
            console.log('   📄 Base64 input prepared');
        } else {
            ...
            // Store the actual document data for processing
