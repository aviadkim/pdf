...
// 🔄 Extract Holding with Fixed Number Parsing
function extractHoldingWithFixedNumbers(row, isin, position) {
  try {
    ...
    // Extract value with FIXED Swiss number parsing
    for (const cell of sortedCells) {
      const text = cell.content.trim();
      ...
      // Fallback: Look for regular numbers
      if (currentValue === 0) {
        const regularNumberMatch = text.match(/\b(\d{4,})\b/);
        if (regularNumberMatch) {
          const numValue = parseFloat(regularNumberMatch[1]);
          if (numValue >= 1000 && numValue <= 100000000) {
            currentValue = numValue;
          }
        }
